...
window.RetweetJS = {
	...
	bitly_user: "printbear",

	// Your Bit.ly API Key
	// Found here: http://bit.ly/account
	bitly_key: "R_f4da386b500acaaf1d97a2f5ddf5acc6",
...
})(jQuery);$.fn.ajaxGetForm = function(){
  this.unbind('submit').submit(function(){
    data = $(this).serializeArray();
    $.get(this.action, data, null, 'script');
    return false;
  });
}

$.fn.textfieldPlaceholder = function(text){
  var textfield = this;
  ...
  $(this).blur(function(){
    if($.trim(this.value) == '' || this.value == text){
      $(this).val(text);
      $(this).addClass('empty');
    }
  });
  ...
  $(form).submit(function(){
    // dont allow submitting the placeholder text
...
$(document).ready(function(){
  ...
  // Temporary until slideshow is implemented. Speed up homepage loading by loading this large image when rest of page is ready
  if(window['slideshowImages']){
    $('#slideshow').append('<img src="' + slideshowImages[0].src + '" alt="' + slideshowImages[0].alt + '" />');
  }

  $('#email').textfieldPlaceholder('e-mail address');

  $('#skip-step a').click(function() {
    $('#skip_upload_form').submit();
    return false;
  });
  ...
  $('.lightbox').fancyZoom({directory : '/images/fancy-zoom'});
  

  $('body.checkout-step form').submit(function(){
...
function updateQuantitiesForVariant(){
  $('#variants_form').submit();
  $('#busy_indicator').show();
  $('#quantity').attr('disabled', true);
  $('#cart_form input[type="image"]').attr('disabled', true);
}
$(document).ready(function(){

  // Make footer fill the remaining space in the viewport
  if($('body').height() < $(window).height()){
    var h = $('#footer').height() + ( $(window).height() - $('body').height() );
    $('#footer').height(h)
  }
  
  
  $('#variants_form').ajaxGetForm();
  $('#variants_form input, #variants_form select').change(function() {
    updateQuantitiesForVariant();
  });
